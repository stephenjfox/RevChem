...
def correct_realeye_df_group(
    ...
    # Trial is started relative to the first output "test_created_at" rather than the 
    # recording which is output ~1 sec later (and thusly has "test_created_at" with an additional second)
    ...
    # FIXME(stephen): you could make this idempotent rather than stateful at no cost
    # update the dfs
    output_dfs = []
    for group_member_index in range(len(group_dfs)):
        ...
        # second step, because concurrently causes column existence race condition.
        df = df.with_columns(
...
_INFERRED_CU_CUA_CORRECT_INDICES = [0, 1, 2, 3, 5, 6, 4, 9, 7, 8]
...
def pipeline_raw_realeye_to_timed_dataframe(
    ...
    *,
    item_ids_in_order: list[str], # item_ids, ordered as presented by RealEye. Any other order will lead to useless downstream e.g. video output
    ...
    real_eye_rows: list[RealEyeRawRow] = sorted(
        ...
        # NOTE(stephen): the file should already be ordered this way....
        key=lambda re_row: item_ids_in_order.index(re_row.item_id),
    )
    # rows to dataframe
    dfs = [
    ...
    if debug:
        display("## After parsing and typing the RealEye data into DataFrames")
        display(pl.concat([df.head(1) for df in dfs]))

    # group by the participant
    grouped = GroupedFrames.from_tuples((df["participant_id"][0], df) for df in dfs)

    # sort by the stimulus, for a given participant
    dfs_by_p_id_sorted = grouped.apply(
        lambda _, group_dfs: correct_realeye_df_group(
            sorted(
            group_dfs, key=lambda df: item_ids_in_order.index(df["stimulus_id"][0])
        ), time_col=dt_timestamp_col)
    )
    # we're totally ignoring the time aspect here, because being in stimulus order
    # should put all things right.
    grouped = dfs_by_p_id_sorted
...
@patch
def rekeyed(self: GroupedFrames, func: Callable[[K, list[DF]], R], **kwargs) -> 'GroupedFrames':
    """Recreates this instance, with a new key desired from the key and the group

    Args:
        func: compute the key based on the current key and group.

    Returns:
        GroupedFrames instance with the same values as this instance, but new keys as described
    """
    with_new_keys = {
        func(key, frames, **kwargs): frames
        for key, frames in self.items()
    }
    return GroupedFrames(with_new_keys)
